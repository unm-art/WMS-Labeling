...
class CaptionTest extends \PHPUnit_Framework_TestCase
{
    ...
    function testGetCaption(){
    	...
    	$captions = $holdings[0]->getCaptions();
    	$this->assertInstanceOf('WorldShare\WMS\Caption', $captions[0]);
    	return $captions[0];
    }
    
    /**
     * can parse Caption strings
     * @depends testGetCaption
     */
    function testParseLiterals($caption)
    {
    	$this->assertNotEmpty($caption->getId());
    	$this->assertNotEmpty($caption->getSequence());
 
    }
    
    /**
     * can parse Caption objects
     * @depends testGetCaption
     */
    function testParseObjects($caption)
    {
    	$this->assertInstanceOf('WorldShare\WMS\Enumeration', $caption->getEnumerations());
    	$this->assertInstanceOf('WorldShare\WMS\Chronology', $caption->getChronologies());
    }
